# Camunda Db Schema generator
Reducing Boilerplate Code with camunda schema maven plugin
> More Time for Feature and functionality
  Through a simple generator camunda schema and saving 60% of development time 

## Key Features
* Auto generate by maven validate phase
* Auto generate db schema within camunda dependencies
* Ready for use with Flyway maven plugin
...
<plugins>
    <plugin>
        ...
        <artifactId>camunda-schema-maven-plugin</artifactId>
        <version>LATESTS</version>
        <executions>
            <execution>
                <id>camunda-schema-generate</id>
...
## How to override default properties

 | Property | Short Description | Default Value |
 |-------- | ----------------- |----- |
 | schema | DB name | oracle |
 | userName | DB user name | CAMUNDA_app |
 | outputPath | output path for generated sql script | /src/main/resources/db/migration/V1.0__Create_Tables.sql |

## Output 
> V1.0__Create_Tables.sql 
> NOTE: This is an example file for oracle database within maven dependency org.camunda.bpm:camunda-engine:7.12.0.
```
--
...
GRANT SELECT,INSERT,UPDATE,DELETE ON ACT_HI_CASEACTINST TO userName;
GRANT SELECT,INSERT,UPDATE,DELETE ON ACT_HI_CASEINST TO userName;
GRANT SELECT,INSERT,UPDATE,DELETE ON ACT_HI_COMMENT TO userName;
GRANT SELECT,INSERT,UPDATE,DELETE ON ACT_HI_DECINST TO userName;
GRANT SELECT,INSERT,UPDATE,DELETE ON ACT_HI_DEC_IN TO userName;
GRANT SELECT,INSERT,UPDATE,DELETE ON ACT_HI_DEC_OUT TO userName;
...
GRANT SELECT,INSERT,UPDATE,DELETE ON ACT_RU_CASE_SENTRY_PART TO userName;
...
[![Quality Gate Status](https://sonarcloud.io/api/project_badges/measure?project=mtema_camunda-schema-maven-plugin&metric=alert_status)](https://sonarcloud.io/dashboard?id=mtema_camunda-schema-maven-plugin)

[![Coverage](https://sonarcloud.io/api/project_badges/measure?project=mtema_camunda-schema-maven-plugin&metric=coverage)](https://sonarcloud.io/dashboard?id=mtema_camunda-schema-maven-plugin)

[![Technical Debt](https://sonarcloud.io/api/project_badges/measure?project=mtema_camunda-schema-maven-plugin&metric=sqale_index)](https://sonarcloud.io/dashboard?id=mtema_camunda-schema-maven-plugin)
